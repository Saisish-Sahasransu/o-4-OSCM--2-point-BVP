...
for j = 2*(N(p)/10)+1:3*2*(N(p)/10)
    exact(j) = exp(-(x(j))^2);
    exact1(j) = -2*x(j)* exp(-(x(j))^2);
end
for j = 3*2*(N(p)/10)+1:N(p)+1
    exact(j) = cos(x(j));
    exact1(j)= -sin(x(j));
end
error(p) = max(abs(Asol-exact))
error1(p) = max(abs(Asol1-exact1))
end
for j = 1:p-1
...
xlabel('x')
ylabel('Y')
title('Numerically Computed Vs Exact Soln')
...
figure(4)
plot(log(h),log(error1),'*',log(h),log(error1));
xlabel('log(h_i)')
ylabel('log(e_i)')
title('Order of Convergence Graphically')
