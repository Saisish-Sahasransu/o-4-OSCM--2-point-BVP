...
b(5*N(p)+2) = -pi;
for j = 1:N(p)/2
    b(5*j-3) = (-pi^2+kmin)*sin(pi*xi(3*j-2));
    b(5*j-2) = (-pi^2+kmin)*sin(pi*xi(3*j-1));
    b(5*j-1) = (-pi^2+kmin)*sin(pi*xi(3*j));
    b(5*j) = 0;
    b(5*j+1) = 0;
end
for j = N(p)/2+1:N(p)
    b(5*j-3) = (-pi^2+kmax)*sin(pi*xi(3*j-2));
    b(5*j-2) = (-pi^2+kmax)*sin(pi*xi(3*j-1));
    b(5*j-1) = (-pi^2+kmax)*sin(pi*xi(3*j));
    b(5*j) = 0;
    b(5*j+1) = 0;
end
s = A\b;
